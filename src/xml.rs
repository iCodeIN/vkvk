#![allow(unused_labels)]
//! Types and functions to interact with XML.
...
pub fn drop_declaration(words: &str) -> &str {
  ...
  panic!("Illegal: No XML declaration to remove.")
}
...
pub enum XmlElement<'s> {
  ...
  EndTag { name: &'s str },
  /// A tag with no inner span, open and close as one unit.
        ...
        if it.peek()?.1 == '!' {
          ...
          'gather_tag_attrs: loop {
            match it.next()? {
              (i, '/') => match it.next()? {
                (_j, '>') => {
                  let attrs = &self.words[name.len() + 2..i].trim();
                  match it.next() {
                    Some((k, _)) => {
                      self.move_to(k);
                    }
...
impl<'s> Iterator for AttributeIterator<'s> {
  ...
  fn next(&mut self) -> Option<Attribute<'s>> {
    let mut it = self.words.char_indices().peekable();
    let key = loop {
      match it.next()? {
        (i, '=') => {
